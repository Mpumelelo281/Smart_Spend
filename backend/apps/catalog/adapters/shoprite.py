"""Shoprite product search adapter — same shape and caveats as
checkers.py (see that module's docstring): a configurable JSON search
endpoint (SHOPRITE_API_BASE_URL), inert until configured, with a
`_normalize` field mapping that must be verified against the real endpoint
before this is enabled anywhere.
"""
...
class ShopriteAdapter(RetailerAdapter):
    name = "shoprite"
    _timeout_seconds = 8

    def is_configured(self) -> bool:
        return bool(settings.SHOPRITE_API_BASE_URL)

    def search(self, query: str) -> list[NormalizedListing]:
        if not self.is_configured():
            raise AdapterUnavailable("SHOPRITE_API_BASE_URL is not set.")

        try:
            data = self._call_api(query)
        except pybreaker.CircuitBreakerError as exc:
            raise AdapterUnavailable("Circuit open: Shoprite has failed repeatedly recently.") from exc
        except requests.RequestException as exc:
            raise AdapterUnavailable(f"Shoprite request failed: {exc}") from exc
    ...
    def _call_api(self, query: str) -> dict:
        headers = {}
        if settings.SHOPRITE_API_KEY:
            headers["Authorization"] = f"Bearer {settings.SHOPRITE_API_KEY}"
    ...
    def _normalize(raw: dict) -> NormalizedListing | None:
        ...
        return NormalizedListing(
            ...
            retailer_base_url="https://www.shoprite.co.za",
        )
